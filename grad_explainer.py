...
def grad_node_explanation(model, node, x, edge_index):
    ...
    node_grad = torch.nn.Parameter(torch.ones(num_nodes, device=device))
    feature_grad = torch.nn.Parameter(torch.ones(num_features, device=device))
    ...
    # Ensure node is also on the correct device if it's a tensor
...
#     E = edge_index.size(1)
#     edge_mask = torch.nn.Parameter(torch.ones(E, device=device))
